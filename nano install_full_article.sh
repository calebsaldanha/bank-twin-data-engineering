...
# 1. RESTAURAÇÃO DE BACKUP (Para evitar duplicação se você já rodou scripts anteriores)
# Se existirem backups (.bak), restauramos eles para começar de um estado limpo.
# Se não existirem, criamos backups dos arquivos atuais.
...
# ==========================================
# Aqui inserimos o texto exato que você enviou, formatado para HTML.
cat > article_content.tmp << 'EOF'
...
# Remover links antigos do MathJax se existirem
sed -i '/MathJax-script/d' index.html
sed -i '/polyfill.io/d' index.html

# Configuração ROBUSTA do MathJax (Correção Visual)
cat > mathjax_config.tmp << 'EOF'
    <script>
    window.MathJax = {
      tex: {
        inlineMath: [['$', '$'], ['\\(', '\\)']],
      ...
      startup: {
        typeset: false // Importante para modais
      }
    };
    </script>
